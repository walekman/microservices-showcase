 ...
 * not a genuine bearer header). The decoder accepts two canned token strings: CUSTOMER_TOKEN
 * carries every capability permission, mirroring what a real "customer" composite-role token
 * would contain, and ADMIN_TOKEN carries account-admin only, mirroring the admin persona Phase
 * 7b introduced. There's no real signature to verify since no real Keycloak is involved in
 * these tests. See docs/phase-7-auth-keycloak-jwt.md and
