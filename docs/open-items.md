...
|---|---|---|---|
| 1 | **A debit that times out during a live transfer is never reconciled.** It is recorded `FAILED` + `ACCOUNT_SERVICE_UNAVAILABLE`, but `CompensationScheduler` sweeps only `COMPENSATION_REQUIRED` and stale `PENDING`. A debit that actually committed leaves the source account without the money, and the transfer is never corrected. | `microservices-showcase-design.md` §4 | Open |
...
| 4 | **`POST /transfers` has no idempotency key.** A double-submit from a caller creates two transfers. It was deferred "until a real caller exists", and the Bank UI is now that caller. | `phase-3-resilience-compensation-idempotency.md` Scope Boundary | Open, and the stated trigger has passed |
