...
- Flyway vs. `ddl-auto` for schema management — revisit before Phase 3 or later deploys to two service schemas (Account + Transfer). Currently using `ddl-auto: update`.
- Idempotency keys for debit/credit — Phase 3 item; relevant once Transfer's saga can retry a sync call into Account via Resilience4j.
