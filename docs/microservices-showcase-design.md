...
- **Transfer left `PENDING`** (e.g. the process died mid-saga): a stale-`PENDING` sweep replays the debit key to learn whether the debit landed — `FAILED` if it did not, promoted to `COMPENSATION_REQUIRED` (then resolved as above) if it did.
...
- **Account/Fraud unavailable after retries exhausted:** the circuit breaker opens and Transfer Service fails fast (`503`, no hang). On the debit leg the transfer is recorded `FAILED` + `ACCOUNT_SERVICE_UNAVAILABLE`, meaning *debit not confirmed*, never *debit did not happen*.
  - **Known gap:** neither sweep revisits these rows — the scheduler queries only `COMPENSATION_REQUIRED` and stale `PENDING` — so a debit that timed out but actually committed is never reconciled automatically.
