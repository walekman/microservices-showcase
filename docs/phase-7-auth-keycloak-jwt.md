...
- **Re-synced from implementation (see roadmap.md's Phase 7 deferred-items entry for the full account):** this phase's Testing section originally planned Testcontainers-backed Keycloak in the automated suite. That was not implemented — each service instead gets a stub `JwtDecoder` (`@TestConfiguration`, `@Primary`, named distinctly from the production `jwtDecoder` bean to avoid a same-name registration collision) returning canned `Jwt`s, which proves the authorization matrix without a real Keycloak. The realm-export JSON's actual import behavior and the full login → Gateway → service round trip against a real Keycloak remain unverified by the automated suite — only a real `docker compose up` proves those.
